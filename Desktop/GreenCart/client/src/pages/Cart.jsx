...
const Cart = () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.addresses.length > 0) {
          setSelectedAdres(data.addresses[0]);
        }
      } else {
  ...
  const placeOrder = async () => {
    try {
      if (!selectedAdres) {
        return toast.error("pls select the address");
      }
          ...
          >
            ...
            >
              <img
                ...
                alt="remove"
                className="inlice-block h-6 w-6"
              />
            </button>
          </div>
        ))}
        ...
        <div className="mb-6">
          ...
          <div className="relative flex justify-between items-start mt-2">
            <p className="text-gray-500">
              {selectedAdres
                ? `${selectedAdres.street}, ${selectedAdres.city} , ${selectedAdres.country}, ${selectedAdres.state}  `
                : "No Adress found"}
            </p>
            <button
            ...
            {showAddress && (
              <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                {addresses.map((adre, index) => (
                  <p
                    key={index}
                    onClick={() => {
                      setSelectedAdres(adre);
                      setShowAddress(false);
                    }}
                    className="text-gray-500 p-2 hover:bg-gray-100"
                  >
                    {adre.street}, {adre.city} , {adre.state} , {adre.country}
                  </p>
                ))}
                <p
